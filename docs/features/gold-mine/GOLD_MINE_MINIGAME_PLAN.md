...
**Status:** Phase 1 spec — implemented with path drift notes. Current backend routes live under `/api/minigames/gold-mine/*`, and several planned Swift component files were consolidated into `GoldMineMiniGameView.swift` / `GoldMineShaftPicker.swift`. Historical note: the exploratory `gold_mine_minigame_prototype.html` was removed during later repository cleanup.
**Date:** 2026-04-11
**Dependencies:** `GOLD_MINE_MINIGAME_BALANCE_AUDIT.md` and the historical prototype/audit trail that informed this plan
