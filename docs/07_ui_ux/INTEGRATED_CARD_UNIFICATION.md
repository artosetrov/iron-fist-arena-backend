...
4. **Portrait animations** (`glowPhase` rotate + `shimmerOffset` sweep) — идут бесконечно. На экране-листе лидерборда несколько карт одновременно. Они уже не нарушают no-scale rule, потому что это opacity/gradient-only motion. Но **если несколько карт в списке → могут быть FPS-просадки**. Предлагаю: в `LeaderboardPlayerDetailSheet` (single card в sheet) — анимации включены, в потенциальных будущих list-контекстах — параметром `animated: Bool = true` отключаемо. **Подтверди.**
