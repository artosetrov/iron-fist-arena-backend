...
По правилу этого review-потока (сначала отчёт + варианты, потом код) и прямому указанию Артёма *«UX reviews must produce report + prototype FIRST, discuss with user, then implement only on approval»* я обязан **сначала показать находки, предложить варианты, получить одобрение — и только потом писать код**.
