...
Правило этого review-потока: сначала отчёт + варианты, потом код. Как и для W2, я снял реальный срез `backend/src/lib/game/balance.ts`, `combat.ts`, `skills.ts`, `elo.ts`, `premium.ts`, схемы Prisma и `backend/tests/` — и сверил с тем, что написано в `QA_FIX_PLAN_2026-04-10.md` (строки 896–1180).
