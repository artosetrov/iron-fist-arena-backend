...
## RPG Sub-Genre & Target Audience

Hexbound is an **Action RPG** (real-time combat, abilities, equipment progression) with competitive PvP focus. This means:

- **Expected mechanics:** real-time combat, spells/abilities, defensive actions (stance system), skill tree, equipment upgrades, item rarity tiers
- **Excluded by design:** turn-based combat, isometric grid, party management, branching dialogue, open-world exploration
- **Target audience (priority order):** Self-Expression (build optimization) > Collection & Completion (items, achievements, passive nodes) > Progression (visible power growth) > Narrative (minimal, lore as flavor)

When designing features, ask: does this serve our Action RPG identity and our audience's primary motivation (Self-Expression through builds)?

---

## Game System Design Philosophy

Good game systems are a **craft**, not just an art. Systems are a tool that designers use for creating, tweaking, tuning, and managing content. Every system in Hexbound must satisfy five qualities:

### The Five Hallmarks

1. **Comprehensible** — You understand all parts of the system: how values are chosen, why they work that way, and what other rules/content they impact. If you want to adjust gameplay, you know exactly what to change.

2. **Consistent** — Rules and content function the same everywhere. Armor doesn't work differently in dungeons vs PvP. Gold formulas don't secretly change at high levels. No silent special-casing.

3. **Predictable** — You can determine how the system behaves in new circumstances. Multiply XP by 2x? You can calculate the result in advance using documented formulas.

4. **Extensible** — New content types plug in easily. New class? New item rarity? New dungeon type? The existing rules accommodate them without rewriting core logic.

5. **Elegant** — Rich tactical situations from a small number of moving parts. The stance system (3 zones × attack/defense) is a good example. Before adding a new parameter — ask if existing ones can combine to achieve the same depth.

### System Taxonomy: Parameters → Rules → Content

Always think in three layers when designing or modifying systems:

- **Parameters** — The atoms: STR, AGI, HP, armor, critChance, gold, stamina, XP, etc. Keep the list lean — every new parameter adds complexity everywhere.
- **Rules** — The formulas: damage pipeline, XP curve, ELO rating, drop chances. No more complex than necessary. Simple inputs → clear outputs.
- **Content** — The things: skills, items, dungeons, bosses, quests. Content is the most extensible layer — hundreds of items shouldn't require rule changes.

### Design Order for New Systems

1. Choose parameters (what values does this need?)
2. Design rules (simplest formulas that implement the vision)
3. Define progressions (how do parameters change over time?)
4. Design content types (as complex and interesting as possible)
5. Add new layers as needed (iterate, don't over-design upfront)

### Systems Follow Gameplay

Systems are not the starting point. Before designing any new system, answer:
- What is the core game loop? → Short-session PvP combat with character progression
- What interesting decisions do players make? → Stat allocation, skill loadout, stance, equipment, resource spending
- What are the appeals? → Competitive ranking, visible power growth, collection, dark fantasy aesthetic

If a new system doesn't serve at least one of these — it's bloat, not depth. **Form follows function, systems follow gameplay.**
...
### System Quality Gate (Five Hallmarks)

When proposing any new mechanic, formula change, or content type — verify:

- [ ] **Comprehensible** — Can you trace the full impact chain of this change?
- [ ] **Consistent** — Does this rule work the same in all contexts (PvP, dungeon, training)?
- [ ] **Predictable** — Can you calculate the outcome with a worked example?
- [ ] **Extensible** — Will this still work when we add new classes, items, skills?
- [ ] **Elegant** — Are you achieving this with minimum new parameters?

Reference: `docs/09_rules_and_guidelines/DEVELOPMENT_RULES.md` → "Game System Design Principles"

## Loot & Randomness Awareness

When designing or reviewing systems with random outcomes (drops, rewards, gacha):

- **Risk ↔ Reward:** High-effort content must never drop only commons. Apply rarity floors based on difficulty.
- **Fixed vs. Random:** Quest rewards and boss drops = fixed/known. Dungeon chests and bonus loot = random with rarity floor.
- **Pity protection:** After $N$ failed attempts at a rare drop, escalate probability or guarantee. Critical for retention.
- **Item comparison UX:** Always show stat delta vs. equipped item ("+5 STR", "−2 AGI") — players shouldn't navigate away to compare.

Reference: `docs/09_rules_and_guidelines/DEVELOPMENT_RULES.md` → "Loot & Randomness Design Principles"
