...
export async function GET(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to fetch dungeon'
...
export async function PUT(
  ...
  try {
    ...
    // Use a transaction to replace all nested data
    const dungeon = await prisma.$transaction(async (tx) => {
      // Delete existing nested records
      await tx.bossAbility.deleteMany({
        where: { boss: { dungeonId: id } },
      })
      await tx.dungeonBoss.deleteMany({ where: { dungeonId: id } })
      await tx.dungeonWaveEnemy.deleteMany({
        where: { wave: { dungeonId: id } },
      })
      await tx.dungeonWave.deleteMany({ where: { dungeonId: id } })
      await tx.dungeonDrop.deleteMany({ where: { dungeonId: id } })

      // Update dungeon + recreate nested records
    ...
    const message = error instanceof Error ? error.message : 'Failed to update dungeon'
...
export async function DELETE(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to delete dungeon'
    return NextResponse.json({ error: message }, { status: 400 })
  }
}
