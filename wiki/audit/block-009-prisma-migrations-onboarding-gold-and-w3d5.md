...
| `backend/prisma/migrations/20260410_add_tutorial_completed/migration.sql` | Tutorial replay-prevention delta | Adds `tutorial_completed` and `tutorial_completed_at` plus a partial index. | Used by scripted tutorial fight routes. | Intended to become the replay-prevention flag. | Standalone migration was incomplete for existing/skip users because it did not backfill from prior tutorial state, but that gap was later closed by `20260415_backfill_tutorial_completion_state` and aligned replay guards. | Fixed |
