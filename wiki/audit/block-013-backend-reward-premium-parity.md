...
- **Status:** Gold-bonus parity is now consistent across reward routes. The original guest-account merge tail was later closed for wallet/premium-card state in [[block-186-backend-guest-oauth-wallet-merge-parity]].
...
| `backend/src/app/api/auth/upgrade-guest-oauth/route.ts` | Guest → OAuth account migration | Links a guest account to Google/Apple and transfers owned data to the OAuth user. | Depends on auth, Supabase admin client, Prisma, rate limit. | Must preserve paid entitlements and wallet/card state when reparenting user-owned rows. | Fixed missing subscription-row transfer and preserved the longer `premiumUntil` when the OAuth user already exists. Later follow-up in [[block-186-backend-guest-oauth-wallet-merge-parity]] also made wallet merge deterministic (`gold`/`gems` sum, later premium claim date wins, longer-lived daily gem card wins). | Fixed |
...
- `social/challenges` is no longer blocked by the earlier `any`-heavy route shaping.
- The original guest→OAuth merge-policy tail for wallet and premium-card state was later closed in [[block-186-backend-guest-oauth-wallet-merge-parity]].
...
- Keep push-token dedupe in mind for future guest→OAuth hardening if duplicate `(userId, platform, token)` collisions start showing up.
...
- `wiki/features/auth.md` now documents the current guest→OAuth merge policy for wallet and premium-card state.
- No current backend doc names `PREMIUM_ENTITLEMENT_USER_SELECT` as the canonical select for premium-aware routes.

## Verification

- Original verification in this block caught `social/challenges` warnings that were valid at the time. Follow-up in [[block-157-stale-audit-tail-contraband-and-social-challenges-sync]] confirmed those route-local `any` warnings are no longer present in the live file.
