...
- **Status:** High-risk tooling hazards reduced; the remaining open script tails are now mostly documentation/policy rather than accidental-repo-sweep behavior
...
- A later follow-up block tightened the remaining Git-helper hazard by making tracked-only staging the default and untracked staging an explicit opt-in.
...
| P2 | `git-commit-push.sh` and `git-watcher.sh` still swept the whole working tree with `git add -A`. | Untracked local artifacts, scratch files, or half-finished audit pages could ride along accidentally in an operator commit. | Later follow-up [[block-171-project-git-helper-tracked-only-staging]] switched both helpers to tracked-only staging by default and made untracked inclusion explicit via `--all` / `HEXBOUND_GIT_HELPER_STAGE_ALL=1`. |
...
| `scripts/git-commit-push.sh` | Auto Git helper | Stages local repo changes, commits, pushes current branch, optionally pushes admin subtree. | Depends on Git repo and optional `admin-deploy` remote. Used manually only. | Intended as a convenience helper when operating locally against the project repo. | Hardened repo detection, stale-lock handling, current-branch push, subtree remote checks, and later [[block-171-project-git-helper-tracked-only-staging]] changed the default to tracked-only staging with explicit `--all` / `HEXBOUND_GIT_HELPER_STAGE_ALL=1` for untracked files. | Fixed |
| `scripts/git-watcher.sh` | Trigger-based Git helper | Watches `.git-trigger`, then stages local repo changes, commits, pushes current branch, and optionally pushes admin subtree. | Depends on Git repo, `.git-trigger`, optional `sync-assets.sh`, optional `admin-deploy` remote. Used by `CLAUDE.md`, herald docs, and older retros. | Watcher should be local/operator-controlled and should not silently introduce unrelated file churn. | Hardened path/branch/lock handling, made asset sync opt-in, and later [[block-171-project-git-helper-tracked-only-staging]] changed the default to tracked-only staging with explicit `HEXBOUND_GIT_HELPER_STAGE_ALL=1` for untracked files. | Fixed |
...
- `scripts/git-commit-push.sh` and `scripts/git-watcher.sh` are still local operator tools, not clean project-wide automation. The remaining boundary is intentional workflow scope, not accidental whole-tree staging.
...
- `scripts/git-commit-push.sh` — still a candidate to move to a local/operator toolbox if the team eventually wants all operator automation outside the shared repo.
- `scripts/git-watcher.sh` — same as above, but the old accidental-stage hazard is no longer the primary reason.

## Documentation Missing Or Stale

- `docs/08_prompts/SOUND_CATALOG.md` now needs to stay clearly framed as a historical prompt/acquisition catalog; runtime audio truth belongs to `Hexbound/Hexbound/Persistence/SFXCatalog.swift` and the live bundle.
- `docs/07_ui_ux/ASSET_CONSISTENCY_AUDIT.md` should preserve its findings as historical provenance, not as a claim that the old `512px` sync cap is still the current script behavior.
